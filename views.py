...
from .actions import do_merchant_complete
...
@never_cache
@psa('{0}:shopify_complete'.format('ecommerce_shopify'))
def shopify_auth(request, backend):
...
@csrf_exempt
@psa('{0}:shopify_complete'.format('ecommerce_shopify'))
def shopify_complete(request, backend, *args, **kwargs):
    """Authentication complete view"""
    return do_complete(request.backend, _do_login, user=request.user,
                       redirect_name=REDIRECT_FIELD_NAME, request=request,
                       *args, **kwargs)

# Merchant center connect
@login_required
@never_cache
@psa('{0}:merchant_complete'.format('social_merchant'))
def merchant_auth(request, backend):
    return do_auth(request.backend, redirect_name=REDIRECT_FIELD_NAME)

# Merchant center connect complete
@login_required
@psa('{0}:merchant_complete'.format('social_merchant'))
def merchant_complete(request, backend, *args, **kwargs):
    """Merchant complete connect"""
    return do_merchant_complete(request.backend, 
                                user=request.user, redirect_name=REDIRECT_FIELD_NAME, 
